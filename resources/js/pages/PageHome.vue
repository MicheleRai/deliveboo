<template>
    <div class="grid">
        ...
        <div class="d-flex row text-center m-auto justify-content-center mb-3 flex-wrap gap-2">
            <router-link v-for="categories in arrCategories" :key="categories.id" class="btn btn-success col-2 mx-3 p-1"
                :to="{ name: 'categories', params: { slug: categories.slug } }">{{ categories.name }}</router-link>
        </div>
        <div class="d-flex gap-3 flex-wrap m-auto justify-content-center container">
            <div class="rounded-4 mb-4 me-4 mycard bg-white" style="width: 14rem; height: 24rem;" v-for="user in arrUsers"
                :key="user.id">
